...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profile || profile.role !== 'admin') throw new Error('Forbidden: Only admins can manage users');
    ...
    if (action === 'update') {
      ...
      if (Object.keys(updateData).length > 0) {
        const { error: updateError } = await supabaseClient.auth.admin.updateUserById(id, updateData);
